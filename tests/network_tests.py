...
def test_attempt_cooperative_close_lnd(lnd_pair, event_loop):
...
def test_find_offline_channels_records_disconnected_lnd_peer(lnd_pair, event_loop):
...
def test_electrum_pay_ln_invoice_lnd(lnd_pair, event_loop):
...
def test_electrum_pay_ln_invoice_lnd_returns_false_on_bad_invoice(lnd_pair, event_loop):
...
def test_list_ln_payments_with_labels_lnd(lnd_pair, event_loop):
...
def test_list_onchain_history_lnd_open_and_close_labels(lnd_pair, event_loop):
...
def test_electrum_pay_onchain_lnd_round_trips_label(lnd_pair, event_loop):
...
def test_find_channel_closings_lnd(lnd_pair, event_loop):
...
# Note: `test_lnd_pair_isolation` previously lived here — a pure
# fixture-self-check that asserted "the per-test lnd_pair really is
# fresh." Every other test in this file proves the same invariant
# transitively when it reads channel state on the assumption that
# both channels are active. The standalone check was ~30s of
# regtest boot for no marginal coverage.
